...
def draw_station_map_cartopy(problem, po):
    ...
    for wmap in sc.wavemaps:
        outpath = os.path.join(
            problem.outfolder,
            po.figure_dir,
            "station_map_%s_%i.%s" % (wmap.name, wmap.mapnumber, po.outformat),
        )
        ...
        if not os.path.exists(outpath) or po.force:
            ...
            stations_meta = [
                (station.lat, station.lon, station.station) for station in wmap.stations
            ]
            ...
            if extent:
                ...
                ax.add_feature(
                    cfeature.NaturalEarthFeature(
                        ...
                        name="ocean",
                        scale="50m",
                        **cfeature.OCEAN.kwargs
                    )
                )

                gl = ax.gridlines(color="black", linewidth=0.5, draw_labels=True)
